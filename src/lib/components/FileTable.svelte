<script lang="ts">
  import { FileText, Play, Eye, AlertCircle, CheckCircle2, Clock, Search } from '@lucide/svelte';
...
<div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xs">
  <div class="p-4 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div class="flex items-center gap-2">
      ...
      <h2 class="text-sm font-semibold text-white">Danh sách trang & Kết quả OCR ({records.length})</h2>
    </div>
    ...
    <div class="flex items-center gap-2.5">
      <div class="relative">
        ...
        <input
          ...
          placeholder="Tìm tên file hoặc text..."
          class="pl-8 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 w-48"
        />
      </div>
    ...
    <table class="w-full text-left text-xs text-slate-300">
      <thead class="bg-slate-950/60 text-slate-400 font-medium border-b border-slate-800">
        <tr>
          ...
          <th class="px-4 py-3">Ghi chú (note)</th>
        ...
        {:else}
          {#each filteredRecords as rec}
            <tr class="hover:bg-slate-800/30 transition">
              ...
              <td class="px-4 py-3 font-medium text-slate-200 max-w-[200px] truncate" title={rec.fileName}>
                {rec.fileName}
              </td>
              <td class="px-4 py-3">
                {#if rec.status === 'done'}
                ...
                {:else if rec.status === 'processing'}
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20">
                    <Clock class="w-3 h-3 animate-spin" /> Processing
                  </span>
                {:else if rec.status === 'error'}
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20" title={rec.errorMessage}>
                    <AlertCircle class="w-3 h-3" /> Error
                  </span>
                {:else}
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-slate-800 text-slate-400 border border-slate-700">
                    Pending
                  </span>
                {/if}
              </td>
              ...
              <td class="px-4 py-3 text-slate-500 text-[11px] truncate max-w-[120px]" title={rec.note}>
                {rec.note || '-'}
              </td>
              <td class="px-4 py-3 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-1.5">
                  ...
                  <button
                    ...
                    title="Chạy OCR riêng file này"
                  >
                    <Play class="w-3.5 h-3.5 {processingFileId === rec.driveFileId ? 'animate-spin' : ''}" />
                  </button>
                </div>
              </td>
            </tr>
